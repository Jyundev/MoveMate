...
export default function MapView({
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      L.marker([dest.lat, dest.lot], { icon: destMarker })
        .addTo(map)
        .bindPopup(`<b>${destinationName}</b><br/>최종 목적지`, { closeButton: false });

      // 경로선
      L.polyline(
        [[hub.lat, hub.lot], [dest.lat, dest.lot]],
        { color: destColor, weight: 4, opacity: 0.7, dashArray: route.mode === 'WALK' ? undefined : '8, 6' }
      ).addTo(map);

      // 허브·목적지가 모두 보이도록
      map.fitBounds([[hub.lat, hub.lot], [dest.lat, dest.lot]], { padding: [60, 60] });
  ...
  const handleLocate = () => {
    ...
    (mapRef.current as { setView: (latlng: [number, number], zoom: number) => void })
      ...
      <div className="fixed inset-0 z-[200] pointer-events-none">
        ...
        >
          {locationStatus === 'loading' ? (
            ...
            <Locate
              size={20}
              className={locationStatus === 'granted' ? 'text-blue-500' : 'text-gray-400'}
            />
          )}
        </button>
        ...
        <div className="absolute bottom-0 left-0 right-0 pointer-events-auto">
          <div className="mx-4 mb-6 bg-white rounded-2xl shadow-lg p-4">
            ...
            <div className="flex items-center gap-4 pt-3 border-t border-gray-100">
              ...
              <Legend color={MODE_COLOR[route.mode]} emoji="📍" label={destinationName} />
              {route.mode !== 'WALK' && (
